...
const { RoomIdentifiers } = require('./policy_enforcer');

// eslint-disable-next-line max-lines-per-function
export function badgesForUser(userInfo, user, hasLongSession) {
  ...
  if (user) {
    if (user.rooms_practiced) {
      if (_includesAll(user.rooms_practiced, RoomIdentifiers.original)) {
        if (_includesAll(user.rooms_practiced, RoomIdentifiers.onDemand)) {
          ...
          const badge = _pickRandomFromList(RoomIdentifiers.original);
    ...
    if (user.max_concurrent >= 10) {
      badges += `:sparkles: I've practiced concurrently with ${user.max_concurrent -
        1} other people\n`;
    }
    ...
    if (user.quiz_score >= 10) {
      badges +=
        `:question: I've correctly answered ${user.quiz_score} riddles on [#🎶literature-quiz]` +
        '(https://discordapp.com/channels/188345759408717825/505872476903964674)\n';
    }

    if (user.riddles_solved >= 10) {
      badges +=
        `:question: My riddles have been solved ${user.riddles_solved} times on [#🎶literature-quiz]` +
        '(https://discordapp.com/channels/188345759408717825/505872476903964674)\n';
    }

    if (user.recitals && user.recitals.length >= 3) {
  ...
  if (mem.roles.some((r) => r.name === 'Hand Revealed')) {
    badges +=
      ":hand_splayed: I've revealed my hand on [#hand-reveals](https://discordapp.com/channels/188345759408717825/440705391454584834)\n";
  }

  if (environment.contributors.includes(mem.id)) {
    badges +=
      ":robot: I've contributed code to Pinano Bot on [GitHub](https://github.com/pinano-discord/Pinano-Discord-Bot)\n";
  }
  ...
  const effectiveName = (mem.nickname ? mem.nickname : mem.user.username).toLowerCase();
