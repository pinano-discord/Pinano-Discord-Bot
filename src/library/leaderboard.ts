class Leaderboard {
  ...
  async refresh(liveData) {
    const storedData = await this.repository_.loadRowsWithNonZeroKeyValue(this.key_);
  ...
  getPageData() {
    const begin = this.pageLength_ * (this.page_ - 1);
    const end = this.pageLength_ * this.page_;
    return { startRank: begin + 1, data: this.cache_.slice(begin, end) };
  }

  _checkPageOutOfRange() {
    const totalPages = Math.ceil(this.cache_.length / this.pageLength_);
