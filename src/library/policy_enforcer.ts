...
import { Role } from '../commands/helpers';
...
function isTempMuted(member) {
  return member.roles.some((r) => r.name === Role.TEMP_MUTED);
}
...
class PolicyEnforcer {
  ...
  isPracticeRoom(channel) {
    const categoryChan = findChannel(channel.guild, environment.chat_channel).parent;
    return channel.parent === categoryChan && channel.type === 'voice';
  }

  getPracticeRooms(guild) {
    const categoryChan = findChannel(guild, environment.chat_channel).parent;
  ...
  async lockPracticeRoom(guild, channel, member) {
    ...
    try {
      await Promise.all(
        channel.members.filter((m) => m !== member && !m.deleted).map((m) => m.setMute(true)),
      );
    } catch (err) {
  ...
  async unlockPracticeRoom(guild, channel) {
    ...
    try {
      await Promise.all(
        channel.members.filter((m) => !m.deleted && !isTempMuted(m)).map((m) => m.setMute(false)),
      );
    } catch (err) {
  ...
  async applyPolicy(guild, member, oldChannel, newChannel) {
    const deletedRoom = await this.createRemoveRooms(guild);
  ...
  async createRemoveRooms(guild) {
    const rooms = this.getPracticeRooms(guild).sort((a, b) => a.position - b.position);
    const basePosition = rooms.first().position;
    const emptyRooms = rooms.filter((chan) => !chan.members.some((m) => !m.deleted));
    ...
    if (emptyRooms.size === 0) {
      ...
      if (this.deletedEmoji && current.unix() - this.deletedAt < 5 * 60) {
      ...
      const channel = await guild.createChannel(`Practice Room ${identifier}`, {
        type: 'voice',
        parent: categoryChan,
        bitrate: environment.dev_mode ? 96000 : 384000,
      ...
      const emptyRoom = emptyRooms.find(
        (c) => c.position >= basePosition + environment.minimum_rooms,
      );
      if (emptyRoom) {
  ...
  async applyPermissions(guild, member, channel) {
    const prChat = findChannel(guild, environment.chat_channel);
    if (!prChat) {
      return;
    }
    ...
    if (
      ...
      const overwrite = prChat.permissionOverwrites.get(member.id);
      if (overwrite) {
  ...
  enforceAutolock(guild) {
    this.getPracticeRooms(guild)
      .filter((chan) => chan[!'locked_by'])
      .forEach((chan) => {
        const members = chan.members.filter((m) => !m.deleted);
  ...
  async resetBitrateIfEmpty(channel) {
    const defaultBitrate = environment.default_bitrate;
    if (channel && !channel.members.some((m) => !m.deleted) && channel.bitrate !== defaultBitrate) {
